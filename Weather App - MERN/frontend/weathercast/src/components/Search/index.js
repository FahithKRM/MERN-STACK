...
// import {GoSearch} form 'react-icons/go';
...
const Search = ({ action }) => {
  ...
  return (
    <div
      className={`w-[60%] h-full bg-gray-800 rounded-2x1 rounded-r-3x1 bg-coover bg-center bg-[url('https://www.leitrimobserver.ie/resizer/800/-1/true/1506501941937.jpg--weekend_weather_forecast___more_rain_on_the_way.jpg?1506501942000')]`}
    >
      {/* Search Nav Bar */}
      <div className="w-full h-[18%] flex justify-between item-center py-3 px-4">
        <p className="text-lg text-white"> Forecast </p>
        <div className="flex justify-between items-center bg-black-rgba backdrop-blur-sm rounded-2x1 w-[40%] px-2">
          <MdLocationOn color='#fff' className='font-bold' size={30} />
          <div className="w-[85%] flex flex-col leading-5">
            <p className="text-[0.69rem] -mb-1 text-gray-400">
              Current Location
            </p>
            <p className="text-lg text-white">Sri Lanka, Asia</p>
          </div>
        </div>
      </div>
      ...
      <div className="h-[70%] w-full items-center flex flex-col justify-center">
        <h2 className="text-lg text-white font-bold shadow-inner">
          Welcome to Genius Weather forecast
        </h2>
        <div className="h-[0.2rem] w-28 my-4 rounded-full bg-white"></div>
        <form
          method="post"
          className="w-[80%] mx-auto flex items-center justify-around"
